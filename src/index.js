...
class App extends React.Component {
	...
	bubbleSort(arrayToSort) {
		...
		bubbleSorter(arrayToSort)
			.then(newArray =>
				this.setState({
					state: state.update('sortedArray', list =>
						list.push(...newArray),
					),
				}),
			)
	...
	handleSubmit(event) {
		...
		state.get('value').length >= 1 &&
			this.binarySearchMethod(
				state.get('sortedArray'),
				state.get('value'),
			);
	}
		...
		return (
			<main className="App">
				<section>
					...
					<h3>
						A Bubble Sort / Recursive Binary Search on Immutable.JS
						state
					</h3>
